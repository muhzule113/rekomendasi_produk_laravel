...
class AnalisisController extends Controller
{
    public function index()
    {
        ...
        $total_trans       = DB::table('transactions')->whereIn('status_pesanan', ['Selesai', 'Dikirim'])->where('status_pembayaran', 'Dibayar')->count();
        ...
        $coverage          = $ideal_pairs > 0 ? round(($total_pairs / $ideal_pairs) * 100, 2) : 0;

        $simStats = DB::table('product_similarity')
            ->whereColumn('product_a', '<', 'product_b')
            ->selectRaw('MAX(score) as max_s, MIN(score) as min_s, AVG(score) as avg_s, STDDEV(score) as std_s')
            ->first();
        ...
        $similarities = DB::table('product_similarity as ps')
            ...
            ->whereColumn('ps.product_a', '<', 'ps.product_b')
            ->select('p1.nama_product as p1_name', 'p2.nama_product as p2_name',
                     'p1.id_product as p1_id', 'p2.id_product as p2_id',
                     'ps.score', 'ps.co_occurrence', 'ps.source', 'ps.updated_at')
        ...
        return view('admin.analisis', compact(
            'total_produk', 'total_trans', 'total_pelanggan', 'total_rekomendasi',
            'matrix_products', 'total_pairs', 'last_updated', 'ideal_pairs', 'coverage',
            'max_score', 'min_score', 'avg_score', 'std_score',
            'source_stats', 'top_recommended', 'cf_runs',
            'rek_log_summary', 'similarities', 'currentPage', 'totalPages',
            'count_pairs', 'limit_sim', 'offset_sim',
